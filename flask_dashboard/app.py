...
from backend.components import kpi_card, post_card, leaderboard_row, alert_card, pipeline_card, status_badge, digest_card
from backend.data_loader import (
    load_posts, load_volume, load_alerts, load_sentiment_summary,
    load_table_counts, load_null_quality, load_digest, load_drift, load_correlation
)
from backend.analytics import (
    current_velocity, sentiment_breakdown, pulse_score, pulse_label, momentum_pct,
    top_engaging_posts, rolling_sentiment, hourly_activity, pipeline_status,
    zscore_anomalies, top_terms, average_engagement
)
...
PIPELINE_STAGES = ["Hacker News API", "Kafka", "Stream Processor (VADER)", "CSV Store", "PulseLite"]
...
def _check_schema_registry():
    try:
        ...
        latest_id = None
        try:
            v = requests.get("http://localhost:8081/subjects/hn-posts-value/versions", timeout=2).json()
            versions = len(v)
            latest = requests.get(f"http://localhost:8081/subjects/hn-posts-value/versions/{v[-1]}", timeout=2).json()
            latest_id = latest.get("id")
        except Exception:
            pass
        return {"connected": True, "subjects": subjects, "versions": versions, "latest_id": latest_id}
    except Exception:
...
def home():
    ...
    digest_html = None
    if digest:
        digest_html = digest_card(digest["text"], digest["mode"], digest["generated_at"])
    ...
    kpi_cards_html = [
        ...
        kpi_card("⚡", "Velocity", f"{velocity:g}/min", momentum_text, _direction(momentum)),
        ...
        kpi_card("🚨", "Active Anomalies", f"{len(alerts)}", f"{len(zscore_flags)} statistical flags", "down" if len(alerts) else "flat"),
        kpi_card("🔥", "Avg Engagement", f"{avg_engagement:g}", "score + comments", "flat"),
    ]

    # ---- Overview tab charts ----
    volume_chart_data = {
        "x": volume["minute_dt"].dt.strftime("%Y-%m-%d %H:%M").tolist() if not volume.empty else [],
        "y": volume["post_count"].tolist() if not volume.empty else [],
        "rolling": volume["post_count"].rolling(window=min(5, max(len(volume), 1)), min_periods=1).mean().tolist() if not volume.empty else [],
        "anomaly_x": zscore_flags["minute_dt"].dt.strftime("%Y-%m-%d %H:%M").tolist() if not zscore_flags.empty else [],
        "anomaly_y": zscore_flags["post_count"].tolist() if not zscore_flags.empty else [],
    }
    donut_data = {"positive": breakdown.positive, "negative": breakdown.negative, "neutral": breakdown.neutral, "index": breakdown.index}

    terms = top_terms(posts["title"], top_n=12) if not posts.empty else []
    trending_terms_data = {"words": [t[0] for t in terms][::-1], "counts": [t[1] for t in terms][::-1]}
    ...
    feed_items = [
        {
            "html": post_card(
                title=row["title"], score=row["score"], comments=row["comments"],
                sentiment=row["sentiment"], label=row["sentiment_label"],
                timestamp=row["timestamp"], post_id=row.get("id"),
            ),
    ...
    rolling_chart_data = {
        "timestamps": roll["timestamp_dt"].dt.strftime("%Y-%m-%d %H:%M").tolist() if not roll.empty else [],
        "sentiment": roll["sentiment"].tolist() if not roll.empty else [],
        "rolling": roll["rolling"].tolist() if not roll.empty else [],
    }

    scatter_chart_data = {"positive": {"x": [], "y": [], "text": []}, "negative": {"x": [], "y": [], "text": []}, "neutral": {"x": [], "y": [], "text": []}}
    for _, row in posts.iterrows():
        label = row["sentiment_label"] if row["sentiment_label"] in scatter_chart_data else "neutral"
    ...
    heatmap_data = {"z": heat.values.tolist() if not heat.empty else [], "x": [f"{h:02d}:00" for h in heat.columns] if not heat.empty else [], "y": list(heat.index) if not heat.empty else []}
    ...
    drift_before_after = []
    if drift_available:
        drift_chart_data = {"x": drift_df["ts"].dt.strftime("%Y-%m-%d %H:%M").tolist(), "y": drift_df["drift_score"].tolist()}
        ...
        if "before_titles" in drift_df.columns:
            ...
            drift_before_after = [
                {"ts": str(r["timestamp"])[:19], "score": f"{r['drift_score']:.3f}",
                 "before": str(r.get("before_titles", ""))[:150], "after": str(r.get("after_titles", ""))[:150]}
                for _, r in high.iterrows()
            ]

    # ---- Anomalies ----
    alerts_html = [alert_card(row.timestamp, row.post_count, row.rolling_avg) for row in alerts.itertuples()] if not alerts.empty else []
    z_df = zscore_anomalies(volume, threshold=-999)
    zscore_chart_data = {"x": z_df["minute_dt"].dt.strftime("%Y-%m-%d %H:%M").tolist() if not z_df.empty else [], "y": z_df["zscore"].tolist() if not z_df.empty else []}
    ...
    if corr_available:
        corr_chart_data = {"x": list(range(len(corr_df))), "new_volume": corr_df["new_volume"].tolist(), "top_volume": corr_df["top_volume"].tolist()}
        latest_corr = float(corr_df["correlation"].iloc[-1])
        corr_gauge_data = {"value": round(latest_corr, 3)}
        corr_label = "Strong positive" if latest_corr > 0.7 else "Moderate" if latest_corr > 0.3 else "Weak/No correlation"
    ...
    return render_template(
        "index.html",
        app_name=APP_NAME, app_tagline=APP_TAGLINE, app_icon=APP_ICON, source_label=SOURCE_LABEL,
        now_time=datetime.now().strftime("%H:%M:%S"), now_date=datetime.now().strftime("%d %b %Y"),
...
def export_alerts():
    alerts = load_alerts()
    csv_data = alerts.drop(columns=["timestamp_dt"], errors="ignore").to_csv(index=False)
    return Response(csv_data, mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=pulselite_alerts.csv"})
