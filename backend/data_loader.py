...
def load_drift(limit: int = 200) -> pd.DataFrame:
    path = os.path.join(DEMO_DATA_DIR, "data_drift.csv") if DEMO_MODE else "data_drift.csv"
    if not os.path.exists(path):
        return pd.DataFrame()
    try:
        df = pd.read_csv(path, encoding="utf-8")
...
def load_correlation(limit: int = 200) -> pd.DataFrame:
    path = os.path.join(DEMO_DATA_DIR, "data_correlation.csv") if DEMO_MODE else "data_correlation.csv"
    if not os.path.exists(path):
        return pd.DataFrame()
    try:
        df = pd.read_csv(path, encoding="utf-8")
